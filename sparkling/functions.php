...
/**
 * Get the Category ID Number.  It's different between local dev, staging and live environmentites. 
 * Used to exclude Featured posts from News and Video loop on ASN home page
 */
...
function sparkling_setup() {
	...
	add_theme_support( 'post-thumbnails' );
  	...
  	add_image_size( 'sparkling-featured', 540, 432, true );
	add_image_size( 'sparkling-featured-portrait', 432, 540, true );
	add_image_size( 'sparkling-featured-post', 988, 556, array( 'top', 'center') );
	add_image_size( 'sparkling-img-575x400', 575, 400, true );
	add_image_size( 'sparkling-img-405x400', 405, 400, true );
	add_image_size( 'sparkling-img-405x400', 405, 400, true );
	add_image_size( 'sparkling-img-320x400', 320, 400, true );
 ...
 */
  ...
  // Add jQueryUI and Datepicker for Schedule page only if is front page ans slider is enabled
	if(is_page('schedule')) {
		wp_enqueue_script('jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.8.16/jquery-ui.min.js', array('jquery'), '1.8.16');
		wp_enqueue_script('schedule-page', get_template_directory_uri().'/js/schedule-page.js', array('jquery'));
		
		//Station lookup by Zip Code script
		wp_enqueue_script('xxxxx', get_template_directory_uri().'/inc/js/zipcode-to-channel.js');
  	}
  ...
  // Add Bootstrap default JS
	wp_enqueue_script('sparkling-bootstrapjs', get_template_directory_uri().'/inc/bootstrap/js/bootstrap.min.js', array('jquery') );
  ...
  // Main theme related functions
    wp_enqueue_script('date-format', get_template_directory_uri().'/inc/js/date-format.js', array());
  	wp_enqueue_script( 'xxxx', get_template_directory_uri() . '/inc/js/img_resize.js', array('jquery') );
	wp_enqueue_script( 'sparkling-functions', get_template_directory_uri() . '/inc/js/functions.js', array('jquery') );
	wp_enqueue_script( 'scoreboard', get_template_directory_uri() . '/inc/js/asn-scoreboard.js', array('jquery') );
  ...
  //wp_enqueue_script( 'xxxx', get_template_directory_uri() . '/inc/js/asn-scoreboard.js', array('jquery') );
